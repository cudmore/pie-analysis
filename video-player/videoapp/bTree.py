...
class bEventTree(bTree):
	...
	def populateEvents(self, videoFilePath, doInit=False):
		...
		self.eventList = videoapp.bEventList.bEventList(self.myParentApp, videoFilePath)
	...
	def flagOverlap(self, theItem):
		...
		if itemIndex > 0:
			# todo: use treeview.prev() instead
		...
		if itemIndex < len(children) - 1:
			# todo: use treeview.next() instead
	...
	def editNote(self):
		...
		myDialog = videoapp.bDialog.bNoteDialog(self, noteStr, self.editNote_Callback)
